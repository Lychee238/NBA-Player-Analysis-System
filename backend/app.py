...
def create_app():
    ...
    try:
        ...
        def get_players():
            try:
                logger.info("正在获取球员列表...")
                # 直接从数据加载器获取球员列表
                if data_loader.df is None or data_loader.df.empty:
                    logger.error("数据加载器未初始化或数据为空")
                    return jsonify({'error': '数据未加载'}), 500
                
                # 获取唯一的球员名称列表
                players = sorted(data_loader.df['Player'].unique().tolist())
                
                if not players:
                    logger.error("未找到任何球员数据")
                    return jsonify({'error': '未找到球员数据'}), 500
                    
                logger.info(f"成功获取到 {len(players)} 个球员")
                logger.info(f"球员列表示例: {players[:5]}")
                logger.info(f"返回数据类型: {type(players)}")
                
                # 确保返回的是列表
                return jsonify(players)
                
            except Exception as e:
                logger.error(f"获取球员列表时出错: {str(e)}", exc_info=True)
                return jsonify({'error': str(e)}), 500
