    ...
    def get_all_players(self):
        ...
        try:
            if self.df is None or self.df.empty:
                logger.warning("数据集未加载，尝试从CSV文件加载...")
                csv_path = os.path.join('backend', 'services', 'Seasons_Stats.csv')
                if not os.path.exists(csv_path):
                    logger.error(f"找不到数据文件: {csv_path}")
                    return []
                self.df = pd.read_csv(csv_path)
                logger.info(f"成功从CSV加载数据，共 {len(self.df)} 条记录")
            
            # 只获取球员名字列表
            players = sorted(self.df['Player'].unique().tolist())
            logger.info(f"成功获取到 {len(players)} 个唯一球员")
            return players
            
        except Exception as e:
            logger.error(f"获取球员列表时出错: {str(e)}")
            return [] 